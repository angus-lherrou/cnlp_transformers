...
def main():
    ...
    # read-in files
    df = pd.read_csv(
        f"{input_path}{TRAIN_FILE}", sep="\t", usecols=["review", "rating"]
    )
    test = pd.read_csv(
        f"{input_path}{TEST_FILE}", sep="\t", usecols=["review", "rating"]
    )
