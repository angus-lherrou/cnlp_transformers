...
class TemporalDocumentDataset(Dataset):
    ...
    def from_instance_list(cls, inst_list, tokenizer):
        ...
        for (ind, inst) in enumerate(inst_list):
            guid = "instance-%d" % (ind)
            examples.append(
                InputExample(guid=guid, text_a=inst, text_b=None, label=None)
            )
...
async def initialize():
    ...
    app.tokenizer = AutoTokenizer.from_pretrained(model_name, config=config)
    model = CnlpRobertaForClassification.from_pretrained(
        model_name,
        config=config,
        tagger=[True, True, False],
        relations=[False, False, True],
        num_labels_list=[17, 9, 2],
...
def process_tokenized_sentence_document(doc: TokenizedSentenceDocument):
    ...
    for sent_ind in range(len(dataset)):
        tokens = app.tokenizer.convert_ids_to_tokens(
            dataset.features[sent_ind].input_ids
        )
        wpind_to_ind = {}
        timex_labels = []
        event_labels = []
        for token_ind in range(1, len(tokens)):
            ...
            if tokens[token_ind].startswith("Ġ"):
                ...
                timex_labels.append(
                    timex_label_list[timex_predictions[sent_ind][token_ind]]
                )
                event_labels.append(
                    event_label_list[event_predictions[sent_ind][token_ind]]
                )
        ...
        for rel in rels_by_sent.get(sent_ind, []):
            ...
            if arg1 is None or arg2 is None:
                logging.warn(
                    "Tried to build a relation but couldn't align one of the arguments: %s to words %s [null=%s], %s [null=%s]"
                    % (
                        str(rel),
                        sents[sent_ind][arg1_ind],
                        str(arg1 is None),
                        sents[sent_ind][arg2_ind],
                        str(arg2 is None),
                    )
                )
                continue
